...
from app.services import features_quarter, derivatives_sentiment
...
def compute_rule_forecast(
    ...
    if not use_quarter_score:
        ...
        export_sensitive_sectors = ["NIFTY_IT", "NIFTY_PHARMA", "NIFTY_50", "NIFTY_100", "NIFTY_200", "NIFTY_500"]
        ...
        result = {
            ...
            "prob_up": round_to_2_decimal(prob_up) or 0.0,
            "prob_neutral": round_to_2_decimal(prob_neutral) or 0.0,
            "prob_down": round_to_2_decimal(prob_down) or 0.0,
            "expected_return_pct": round_to_2_decimal(expected_return) or 0.0,
    ...
    if quarter_score is not None:
        result["quarter_score"] = round_to_2_decimal(quarter_score) or 0.0
        result["drivers"] = contributions
    
    # Add derivatives sentiment if available
    if db is not None:
        try:
            deriv_sentiment = derivatives_sentiment.compute_derivatives_sentiment(
                db, sector_id, features.date
            )
            result["derivatives_sentiment"] = deriv_sentiment
        except Exception as e:
            logger.warning(f"Failed to compute derivatives sentiment for {sector_id}: {e}")
