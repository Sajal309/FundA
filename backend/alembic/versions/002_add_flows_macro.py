...
def upgrade() -> None:
    ...
    bind = op.get_bind()
    if not bind.dialect.has_table(bind, 'fii_dii_daily'):
        op.create_table(
            'fii_dii_daily',
    ...
    # Create sector_flows_daily table if it does not exist
    bind = op.get_bind()
    if not bind.dialect.has_table(bind, 'sector_flows_daily'):
        op.create_table(
            'sector_flows_daily',
            sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
            sa.Column('date', sa.Date(), nullable=False),
    ...
    # Create macro_daily table if it does not exist
    bind = op.get_bind()
    if not bind.dialect.has_table(bind, 'macro_daily'):
        op.create_table(
            'macro_daily',
            sa.Column('id', sa.BigInteger(), autoincrement=True, nullable=False),
            sa.Column('date', sa.Date(), nullable=False),
